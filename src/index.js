...
const graveMaterial = new THREE.MeshStandardMaterial({ color: '#a9c6b1' });

const gravesPositions = []; // To store the positions of all graves

function checkOverlap(newPosition) {
  for (const pos of gravesPositions) {
    // Assuming a grave's total space including a safety margin is 0.8 meters in both x and z directions
...
for (let i = 0; i < 50; i++) {
  ...
  gravesPositions.push(position); // Store the successful position
}
